    ...
    def _new_layer_with_all_neurons(self):
        ...
        for u1 in range(0, n):
            for u2 in range(u1 + 1, n):
               ...
               # y = gaussian function
                if RefFunctionType.rfGauss in self.param.ref_function_types:
                    layer.add_neuron(u1, u2, RefFunctionType.rfGauss, self.model_class, self.loss)

                if RefFunctionType.rfSigmoid in self.param.ref_function_types:
                    layer.add_neuron(u1, u2, RefFunctionType.rfSigmoid, self.model_class, self.loss)
