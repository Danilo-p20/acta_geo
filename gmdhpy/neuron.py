...
class RefFunctionType(Enum):
    ...
    def get_name(cls, value):
        if value == cls.rfUnknown:
            ...
            return 'Cubic'
        elif value == cls.rfGauss:
            return 'Gauss'
        elif value == cls.rfSigmoid:
            return 'Sigmoid'
    ...
    def get(arg):
        ...
        if arg == 'linear':
            ...
            return RefFunctionType.rfGauss
        elif arg == 'sigmoid':
            return RefFunctionType.rfSigmoid
        else:
...
class PolynomNeuron(Neuron):
    ...
    def _transfer_gauss(self, u1, u2, w , x):
        #x1 = x[:, self.u1_index]
        #x2 = x[:, self.u2_index]
        rho    = w[0]#np.cov(x1,x2)
        
        med_1  = w[1]#np.mean(x1)
        desv_1 = w[2]#np.std(x1)
        
        med_2  = w[3]#np.mean(x2)
        desv_2 = w[4]#np.std(x2)
        
        po = -1 / ( 2 * (1 - rho)) 
        p1 = ((u1 - med_1) / desv_1) ** 2
        p2 = ((u2 - med_2) / desv_2) ** 2
        
        p3 = -2 * rho * (u1 - med_1) * (u1 - med_1) / ( desv_1 * desv_2 )
        p4 = 1 / (2 * np.pi * desv_1 * desv_2 * (1 - rho**2)**(1/2))
        p  = p1 + p2 + p3
        y  = p4 * np.exp(po * (p ))
        return self.activation(y)
    
    def _transfer_sigmoid(self, u1, u2, w):
        return self.activation(w[0]+ 1 / ((1 + np.exp(w[1]*u1) * (1 + np.exp(w[2]*u2)))))
    ...
    def set_type(self, new_type):
        ...
        if new_type == RefFunctionType.rfLinear:
            ...
            self.fw_size = 10
        elif new_type == RefFunctionType.rfGauss:
            self.transfer = self._transfer_gauss
            self.fw_size = 5
        elif new_type == RefFunctionType.rfSigmoid:
            self.transfer = self._transfer_sigmoid
            self.fw_size = 5
        else:
    ...
    def get_name(self):
        ...
        elif self.ftype == RefFunctionType.rfGauss:
            return 'Gaussian function'
        elif self.ftype == RefFunctionType.rfSigmoid:
